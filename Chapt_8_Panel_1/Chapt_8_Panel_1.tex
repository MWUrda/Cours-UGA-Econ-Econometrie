...
\begin{frame}[allowframebreaks]{Exemple: fonction de production agricole}
    \begin{itemize}
        ...
        \begin{align}
            \Delta y_{it} &= a_{1, 0}\Delta_{it} + a_{2, 0}\Delta k_{it} + \Delta \gamma_t +\Delta \varepsilon_{it}.
            \label{eq3}
        \end{align}
...
\end{itemize}
\end{frame}
\begin{frame}[allowframebreaks]{dditivité/non-additivité des erreurs}
        \begin{itemize}
            \item Par exemple:
            \begin{align*}
    ...
    \end{frame}
        ...
        \begin{frame}[allowframebreaks]{Mondèles statiques/non-statiques}
            \begin{itemize}
...
\item Le  modèle peut être présenté comme un sytème de f $T$ equations(une equation pour  $t=1, 2, \ldots, T$) 
...
\begin{frame}[allowframebreaks]{Effets fixes}
 \begin{itemize}
\item Dans cette approche aucune condition contraignant la relation entre $\epsilon_{it}$ et $\{\boldx_{it}\}_{t=1}^T$
 n'est posée. Par exemple, on ne supposera pas que $ \Exp[\boldx_{it}\epsilon_{it}] = 0 $.
